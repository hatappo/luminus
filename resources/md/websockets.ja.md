...
この節では、LuminusのImmutantと HTTP Kit によるクライアント/サーバ間通信のためのWebsocketを使用した例を扱います。最初のステップとして新しいアプリケーションを作りましょう。このサンプルではReagentのcomponentを使うので、`cljs+`profileを入れてください。

Immutantベースのアプリケーションを作るには、Webレイヤにprofileをデフォルトのままに使用してください。

```
lein new luminus multi-client-ws +cljs
```

HTTP Kit ベースのアプリケーションを作るには、デフォルトではなく`+http-kit`profileを使います。

```
lein new luminus multi-client-ws +cljs +http-kit
```

アプリケーションが作成したら、サーバとFigwheelを起動する必要があります。そのためには、それぞれ別々のターミナルにおいて次の2つのコマンドを実行する必要があります。
